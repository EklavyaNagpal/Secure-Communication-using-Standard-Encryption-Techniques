...
// This function produces Nb(Nr1+1) round keys. The round keys are used in each round to decrypt the states.
...
temp[0] =  temp[0] ^ rcon[i/Nk1];

}

else if (Nk1 > 6 && i % Nk1 == 4)
...
RoundKey[i*4+0] = RoundKey[(i-Nk1)*4+0] ^ temp[0];

RoundKey[i*4+1] = RoundKey[(i-Nk1)*4+1] ^ temp[1];

RoundKey[i*4+2] = RoundKey[(i-Nk1)*4+2] ^ temp[2];

RoundKey[i*4+3] = RoundKey[(i-Nk1)*4+3] ^ temp[3];
...
// There will be Nr1 rounds.

// The first Nr1-1 rounds are identical.

// These Nr1-1 rounds are executed in the loop below.

for(round=Nr1-1;round>0;round--)
...
*/
Nr1=128;
// Calculate Nk1 and Nr1 from the received value.
...
/* char *arr;
 char* writablestring= "54 4e 09 7d 6f fd e8 eb 32 22 38 b0 02 7c 5e fb";
...
//in[]="54 4e 09 7d 6f fd e8 eb 32 22 38 b0 02 7c 5e fb";
...
printf("\n");
//char str[]="0000006a6174696e";
  //  convert("4b6174616920746f7567682074686120626861692e");
